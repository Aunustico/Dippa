...
Previously, we discussed of the sensor device hardware and it's primary components. In general, the sensor device is an accelerometer with temperature sensor and data storage. The sensor device is already existing prototype enhanced with a SD card slot and SD card. Conversely, the device was without any software. Thus, implementing a new software from a scratch is required. The objective of the device software is only to record the sensor data on the SD data storage.  
...
The dairy cow estrus is detectable with triaxial accelerometer only and no other sensors are required. The results were verified with one single commercial product. Thus, the results may not be completely reliable and valid.
