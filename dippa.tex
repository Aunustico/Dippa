...
\item \textit{Secure Digital (SD)} \cite{sdspecification} memory card is common consumer product for storing data, typically on portable devices. The advantage of 
\end{itemize}
...
as a micro-controller unit is the fundamental component of the sensors device in this study. It is responsible of execution of the software flow whenever it is powered on. The software flow begins from the boot e.g., after switching the power on or resetting the controller. During the boot, it initializes itself as well as the connected sensors according to the software loaded on its in-system re-programmable flash program memory. After the boot, the micro-controller begins the execution of the software loop of various tasks and events. These tasks and events are discussed in detail in the software section \ref{softwaresection}.  
